...
setwd("G:/My Drive/Work/GitHub/gha_report_2021/datasets/Poverty")

p20thresholds <- fread("Poverty thresholds.csv")
wb_un.regions <- fread("WB_UN regions.csv")
...
modelled <- fread("modelled_countries.csv")
...
if(!("WUP_urban.xls" %in% list.files() & "WUP_rural.xls" %in% list.files())){
  download.file("https://population.un.org/wup/Download/Files/WUP2018-F19-Urban_Population_Annual.xls", "WUP_urban.xls", mode="wb")
  download.file("https://population.un.org/wup/Download/Files/WUP2018-F20-Rural_Population_Annual.xls", "WUP_rural.xls", mode="wb")
}

all.years <- c(min(as.character(projpov$ProjYear)):max(as.character(projpov$ProjYear)))

wup.urb <- read_xls("WUP_urban.xls", skip=16)
wup.rur <- read_xls("WUP_rural.xls", skip=16)
...
name <- paste0("globalproj_long_", format(as.Date(Sys.Date()), "%b%y"), ".csv")
